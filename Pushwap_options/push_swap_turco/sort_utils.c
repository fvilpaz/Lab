...
/*   Updated: 2026/01/13 18:42:43 by fvilas-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_target_position_in_b(t_node *b, int index_to_push)
{
	...
	if (index_to_push < get_min_index(b) || index_to_push > get_max_index(b))
		return (get_pos_of_index(b, get_max_index(b)));
	current = b;
	target_pos = 0;
	while (current)
	{
		...
		if (!next_node)
			next_node = b;
		if (current->index > index_to_push && next_node->index < index_to_push)
			return (target_pos + 1);
...
int    get_target_position_in_a(t_node *a, int index_to_push)
{
    ...
    if (index_to_push > get_max_index(a) || index_to_push < get_min_index(a))
        return (get_pos_of_index(a, get_min_index(a)));
    current = a;
    target_pos = 0;
    while (current)
    {
